...
if not os.path.exists(CHINESE_FONT):
    print(f"警告: 字体文件 {CHINESE_FONT} 不存在！")
    CHINESE_FONT = None
...
class SettingsPopup(ModalView):
    ...
    def __init__(self, **kwargs):
        ...
        title = Label(text="设置每局时长", font_size=32, bold=True,
                      font_name=CHINESE_FONT if CHINESE_FONT else 'Roboto')

        btn_layout = GridLayout(cols=2, spacing=15, size_hint=(1, 0.7))
        for t in [30, 60, 90, 120]:
            btn = RoundedButton(text=f"{t}秒", bg_color=(0.3, 0.6, 1, 1),
                                font_name=CHINESE_FONT if CHINESE_FONT else 'Roboto')
            btn.bind(on_press=lambda x, seconds=t: self.set_time(seconds))
            btn_layout.add_widget(btn)

        close_btn = Button(text="关闭", size_hint=(1, 0.2), background_normal='', background_color=(0.4, 0.4, 0.4, 1),
                           font_name=CHINESE_FONT if CHINESE_FONT else 'Roboto')
    ...
    def set_time(self, seconds):
        App.get_running_app().game_duration = seconds
        self.dismiss()
...
class MyPageScreen(Screen):
    def __init__(self, **kwargs):
        ...
        menu_layout.add_widget(MenuItem("游戏设置 (时长)", lambda: SettingsPopup().open()))
...
class GameScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # === 修复点：添加 font_name 属性，确保数字显示风格一致 ===
        self.timer_lbl = Label(
            text="60",
            font_size=50,  # 稍微加大字体，因为去掉了文字
            color=(1, 0, 0, 1),
            size_hint=(1, 0.1),
            bold=True,
            font_name=CHINESE_FONT if CHINESE_FONT else 'Roboto'  # 加上这句以防万一
        )
        ...
        self.wrong_btn = Button(text="答错", font_size=36, color=(0, 0, 0, 1), background_color=(0, 0, 0, 0),
                                font_name=CHINESE_FONT if CHINESE_FONT else 'Roboto')
        ...
        self.right_btn = Button(text="答对", font_size=36, color=(0, 0, 0, 1), background_color=(0, 0, 0, 0),
                                font_name=CHINESE_FONT if CHINESE_FONT else 'Roboto')
    ...
    def on_leave(self):
        self.stop_sensor(); self.stop_timer()
    ...
    def set_category(self, name, questions):
        ...
        self.seconds_left = App.get_running_app().game_duration
        self.show_question()
    ...
    def update_time(self, dt):
        self.seconds_left -= 1
        # === 修复点：只显示数字，去掉了 "剩余：" 这种中文前缀 ===
        self.timer_lbl.text = str(self.seconds_left)
        if self.seconds_left <= 0: self.game_over()
    ...
    def game_over(self):
        self.stop_timer()
        self.stop_sensor()
        self.q_lbl.text = f"时间到!\n答对: {self.score}"
        self.wrong_btn.disabled = True
        self.right_btn.disabled = True
        Clock.schedule_once(lambda dt: setattr(self.manager, 'current', 'question_bank'), 3)

    def start_sensor(self):
        if accelerometer:
            try:
                accelerometer.enable(); self.sensor_event = Clock.schedule_interval(self.check_tilt, 0.1)
            except:
                pass
    ...
    def check_tilt(self, dt):
        ...
        try:
            ...
            if val and val[2]:
                if val[2] < -7:
                    self.handle_correct(None); self.cooldown()
                elif val[2] > 7:
                    self.handle_wrong(None); self.cooldown()
        except:
            pass
